...
const findValidPasswords = (start, end) => {
  ...
  const hasAdjacentDigits = password => /([0-9])\1/.test(password)
  ...
  for (i = start; i <= end; i++) {
    if (hasAdjacentDigits(i) && hasNoDecrease(i)) validPasswords.push(i)
  }

  return validPasswords.length
}

console.log('puzzle input =>', findValidPasswords(387638, 919123)) // 466
